function [rr,s0,prob,psz]=stpierre_ps(l,doPause)
...
inputDir=fullfile(getenv('HOME'),'photoscan','stpierre');
...
inputFile=fullfile(inputDir,[stub,'.psz']);
...
%s0.estIO(1:6)=true;
% ...but set initial lens distortion values to zero.
s0.IO(3+(1:s0.nK+s0.nP))=0;
...
reportFile=fullfile(inputDir,[stub,'-dbatreport.txt']);
