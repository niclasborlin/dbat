function v=pm_eulerbundle1p_v3all(x,pt,ptCams,IO,nK,nP,EO,cams,OP,vis,PI,constr,orthoP,cIO,cEO,cOP,cPI,cSI,varargin)
...
persistent DEPTH_LIMITS UPDIR LTR OPIX OPID VETO_ACTIVATED CAMID VIS
...
if ischar(x)
    if x(1)=='s'
        ...
        OPID=nP;
        CAMID=EO;
        VIS=false(length(OPID),max(CAMID));
        VIS(:,CAMID)=true;
