function [f,J,JJ]=brown_euler_cam3(x,s)
%BROWN_EULER_CAM3 Residual function for the Brown camera model with Euler angles.
%
%   R=BROWN_EULER_CAM3(X,S) returns the residual vector R of the camera
...
if all(s.IOdistModel==1) % backward/photogrammetric
    if (nargout<2)
        ...
        ld=multilensdistaffine(m,IO,s.nK,s.nP,s.ptCams);
