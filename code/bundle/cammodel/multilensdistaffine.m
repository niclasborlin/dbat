function [ld,dIO,dp]=multilensdistaffine(p,IO,nK,nP,cams,cIO,cp)
%MULTILENSDISTAFFINE Compute affine lens distortion in multiple images.
%
%   MULTILENSDISTAFFINE(P,IO,nK,nP) computes the lens distortion and
%   affine transformation of the points in the 2-by-N array P using
%   the internal orientation parameters in the 16-vector IO. The
%   scalars nK and nP contain the number of radial and tangential
%   distortion parameters, respectively, stored in IO.
%
%   MULTILENSDISTAFFINE(P,IO,nK,nP,CAMS), where IO is 16-by-nCams, and
...
%
%   [LD,dIO,dP]=MULTILENSDISTAFFINE(...) also computes the Jacobians dIO and
%   dP with respect to the IO parameters, and the points P,
%   respectively. If a subset of the Jacobians is wanted, use
%   [LD,dIO,dP]=MULTILENSDISTAFFINE(P,IO,nK,nP,CAMS,cIO,cP), where cIO
...
if ~(any(cIO(:)) || cp)
    ...
    [ixpp,~,ixK,ixP,ixb]=createiocolumnindices(cIO,nK,nP);
    ...
    for i=1:max(cams)
        ...
        [cpp,~,cK,cP,cb]=unpackio(cIO(:,i),nK,nP);
        ...
        [qa,dAdb,dAdc,dAdq]=affine(b,pp,q,1,1,-1);
        ...
        % Lens distortion.
        [lens,dldq,dldpp,dldK,dldP]=browndist(qa,pp,K,P,cp,cpp,cK,cP);
...
fIO=@(IO)vec(multilensdistaffine(p,reshape(IO,16,[]),nK,nP,cams));
fp=@(p)vec(multilensdistaffine(reshape(p,2,[]),IO,nK,nP,cams));
dIO2=jacapprox(fIO,IO(:));
dp2=jacapprox(fp,p(:));
[~,dIO,dp]=multilensdistaffine(reshape(p,2,[]),IO,nK,nP,cams);
mx=-inf;
