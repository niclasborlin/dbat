function [v,dv,dvn]=res_euler_brown_2(Q,q0,ang,f,u,sz,u0,K,P,b,varargin)
...
%   transformation with parameters B (2-by-1) is applied after
...
if nargout<2
    ...
    rhs=affine2(brown_dist(xlat2(aniscale2(scale2(u,sz),[1;-1]),-u0),-K,-P),b);
    v=lhs-rhs;
else
    % Need Jacobians too.
    ...
    [a,dA]=affine2(l,b,cU | cSZ | cU0 | cK | cP,cB);
    v=lhs-a;
end
...
if nargout>1
    ...
    if cU
        dv.dU=-dA.dU*dL.dU*dX.dU*dAS.dU*dS.dU;
    end
    if cSZ
        dv.dSZ=-dA.dU*dL.dU*dX.dU*dAS.dU*dS.dK;
    end
    if cU0
