...
if veto
    fileName=['romabundle2_set',int2str(dataSetNo),'_veto'];
    if vetoAll
    ...
    fileName=['romabundle2_set',int2str(dataSetNo)];
end
...
if exist(fileName,'file') && ~calcProblemData
    ...
    if notequal(D,savedResults.D)
        warning('Different D')
    end
    if notequal(cPert,savedResults.cPert)
        warning('Different cPert')
    end
    if notequal(angPert,savedResults.angPert)
        warning('Different angPert')
    end
    if notequal(veto,savedResults.veto)
        warning('Different veto')
    end
    if notequal(dataset,savedResults.dataset)
        warning('Different dataset')
    end
    if notequal(seed,savedResults.seed)
        warning('Different seed')
    end
    if notequal(host,savedResults.host)
