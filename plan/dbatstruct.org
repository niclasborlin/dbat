...
- IO
  ...
  - struct :: struct with fields
    - block :: 16-by-nImages array with numbering indicating what IO
               ...
               values distinct.
    - lead :: 16-by-nImages logical array indicating what parameters
              are leading a block.
    - uniq :: logical nImages-vector indicating which IOblock columns
              are unique.
    - no ::  nImages-vector with unique numbers for IOblock columns.
    - simple :: logical nImages-vector indicating which IOblock are
...
| IOlead        | IO.struct.lead     |                              |
| IOunique      | IO.struct.uniq     |                              |
| IOno          | IO.struct.no       |                              |
| IOsimple      | IO.struct.simple   |                              |
|---------------+--------------------+------------------------------|
| EO            | EO.val             |                              |
| EOstd         | EO.std             |                              |
| imCams        | EO.cams            |                              |
...
| EOlead        | EO.struct.lead     |                              |
| EOunique      | EO.struct.uniq     |                              |
| EOno          | EO.struct.no       |                              |
| EOsimple      | EO.struct.simple   |                              |
|---------------+--------------------+------------------------------|
| OP            | OP.val             |                              |
| OPstd         | OP.std             |                              |
| OPid          | OP.id              |                              |
...
| sigmas        | post.sigmas        |                              |
|---------------+--------------------+------------------------------|
| paramTypes    | #paramTypes#       | # To trigger a matlab error. |
| prior         | #prior#            |                              |
...
#+BEGIN_SRC perl
  ...
  while (<$in>) {
      if (/^\|[^-]/) {
          ...
          $from =~ s/\./\\./g;
          my $to=$a[2];
          print $out "\ts/$srcVar\\\.$from/$destVar.$to/g;\n";
      }
  }
  print $out "}\n";
  close $in;
